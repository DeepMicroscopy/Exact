...
# ---------------------------------------------------------------------------
# Folder upload — two-step: chunk upload then finalize
# ---------------------------------------------------------------------------
...
@api_view(['POST'])
def upload_folder_chunk(request, imageset_id):
    """Receive one file belonging to a folder-upload batch.

    Required POST fields:
      batch_id       — opaque ID generated by the client (alphanumeric + -_)
      relative_path  — path of this file within the uploaded folder
      file           — the file data
    """
    imageset = get_object_or_404(ImageSet, id=imageset_id)
    if not imageset.has_perm('edit_set', request.user) or imageset.image_lock:
        return Response({}, status=HTTP_403_FORBIDDEN)

    batch_id = request.POST.get('batch_id', '')
    relative_path = request.POST.get('relative_path', '')
    if not _BATCH_ID_RE.match(batch_id):
        return HttpResponseBadRequest('Invalid batch_id')
    if not relative_path or '..' in relative_path or relative_path.startswith('/'):
        return HttpResponseBadRequest('Invalid relative_path')
    if 'file' not in request.FILES:
        return HttpResponseBadRequest('No file attached')

    f = request.FILES['file']
    tmp_dir = Path(tempfile.gettempdir()) / 'exact_folder_upload' / batch_id
...
@api_view(['POST'])
def upload_folder_finalize(request, imageset_id):
    """Process a completed folder-upload batch and create Image records.

    Required POST field:
      batch_id — the same ID used for chunk uploads
    """
    imageset = get_object_or_404(ImageSet, id=imageset_id)
    if not imageset.has_perm('edit_set', request.user) or imageset.image_lock:
        return Response({}, status=HTTP_403_FORBIDDEN)
    ...
    if not tmp_dir.exists():
        return HttpResponseBadRequest('Unknown or expired batch')

    results = []
    errors = []
    try:
        results, errors = _process_folder_upload(imageset, tmp_dir)
    finally:
        shutil.rmtree(str(tmp_dir), ignore_errors=True)

    return Response({'files': results, 'errors': errors}, status=HTTP_200_OK)


def _folder_checksum(dir_path: Path) -> bytes:
    h = hashlib.sha512()
...
def _process_folder_upload(imageset, tmp_dir: Path):
    """Detect folder format and create Image records in imageset."""
    from exact.images.dicom_utils import split_dicom_by_series, series_display_name

    all_files = list(tmp_dir.rglob('*'))
    dcm_files = [f for f in all_files if f.is_file() and f.suffix.lower() == '.dcm']
    mrxs_files = [f for f in all_files if f.is_file() and f.suffix.lower() == '.mrxs']

    results = []
    errors = []

    if mrxs_files:
        for mrxs_src in mrxs_files:
            try:
                dest = Path(imageset.root_path()) / mrxs_src.name
                shutil.copy2(str(mrxs_src), str(dest))

                data_src = mrxs_src.parent / mrxs_src.stem
                if data_src.exists() and data_src.is_dir():
                    data_dest = Path(imageset.root_path()) / mrxs_src.stem
                    shutil.copytree(str(data_src), str(data_dest), dirs_exist_ok=True)

                fchecksum = hashlib.sha512(dest.read_bytes()).digest()
                image = Image(name=mrxs_src.name, image_set=imageset, checksum=fchecksum)
                image.save_file(dest)
                results.append({'name': mrxs_src.name, 'id': image.id, 'type': 'image'})
            except Exception as e:
                logger.error(f'Folder upload MRXS error: {e}')
                errors.append(str(e))

    elif dcm_files:
        series = split_dicom_by_series(tmp_dir)
        if not series:
            errors.append('No readable DICOM series found in the uploaded folder')
        for uid, files in series.items():
            try:
                safe_uid = re.sub(r'[^a-zA-Z0-9._-]', '_', uid)[:64]
                display = series_display_name(files)
                folder_name = re.sub(r'[^a-zA-Z0-9._-]', '_', display)[:64] or safe_uid

                # Avoid collisions
                series_dest = Path(imageset.root_path()) / folder_name
                suffix = 1
                while series_dest.exists():
                    series_dest = Path(imageset.root_path()) / f'{folder_name}_{suffix}'
                    suffix += 1
                series_dest.mkdir()

                for f in files:
                    shutil.copy2(str(f), str(series_dest / f.name))

                fchecksum = _folder_checksum(series_dest)
                image = Image(name=series_dest.name, image_set=imageset, checksum=fchecksum)
                image.save_file(series_dest)
                results.append({'name': series_dest.name, 'id': image.id, 'type': 'image'})
            except Exception as e:
                logger.error(f'Folder upload DICOM series error: {e}')
                errors.append(str(e))

    else:
        errors.append(
            'No recognised folder format found. '
            'Expected a DICOM folder (.dcm files) or a 3DHistech folder (.mrxs file).'
        )
